...
# deconvolve_cloud(7000, 10000, 493, 1000, 64, 100, 30, device, "/mnt/turbo/jfeggerd/outputs_normalized_checkpoints")
            ...
            for i in range(int(xy_size / section_size)):
                for j in range(int(xy_size / section_size)):
                    ...
                    if i > 0 and i < 11 and j > 0 and j < 11:
                        ...
                        output_piece = RL_deconv(img_tensor[:, (i*section_size):((i+1) * section_size), (j*section_size):((j+1) * section_size)], intermediate_output[:, (i*section_size):((i+1) * section_size), (j*section_size):((j+1) * section_size)].type(torch.cdouble), torch.from_numpy(psf_average_small).type(torch.cdouble), iterations = (num + 1) * 10, target_device=device)

            average_output = torch.from_numpy(edge_correction(average_output, 150, 100, (64, 1200, 1200)))



            corrected = np.copy(average_output.numpy().astype(np.uint16))

            # for i in range(int(xy_size / section_size)):
            #     for j in range(int(xy_size / section_size)):
            #         corrected[:, ((i*section_size)):(((i+1) * section_size)), ((j*section_size)):(((j+1) * section_size))] = match_histograms(average_output[:, ((i*section_size)):(((i+1) * section_size)), ((j*section_size)):(((j+1) * section_size))].numpy().astype(np.uint16), img_tensor[:, ((i*section_size)):(((i+1) * section_size)), ((j*section_size)):(((j+1) * section_size))].numpy())

            average_output = torch.from_numpy(corrected)
...
deconvolve_cloud_rolling(6900, 9900, 493, 1200, 64, 1200, 50, device, "/mnt/turbo/jfeggerd/outputs_rolling_edge")